/*---------------------------------------------------------------------------------------------*/
/*  Desc:     Parser Helper for ICCAD 2014 format (verilog, sdc, LEF/DEF)                      */
/*                                                                                             */
  ...
  if (!dot_parm.good())
	...
	else
	{
		...
		while(!dot_parm.eof())
		{
			...
			else if(tmpStr == "GLOBAL_WIRE_CAPACITANCE_PER_MICRON")
				dot_parm >> GLOBAL_WIRE_CAP_PER_MICRON;
			else if(tmpStr == "GLOBAL_WIRE_RESISTANCE_PER_MICRON")
				dot_parm >> GLOBAL_WIRE_RES_PER_MICRON;
	...
	cout <<  "  GLOBAL_WIRE_RESISTANCE  : " << GLOBAL_WIRE_RES_PER_MICRON << " Ohm/um"   << endl;
	cout <<  "  GLOBAL_WIRE_CAPACITANCE : " << GLOBAL_WIRE_CAP_PER_MICRON << " Farad/um" << endl;
	cout <<  "  MAX_WIRE_SEGMENT_LENGTH : " << MAX_WIRE_SEGMENT_IN_MICRON << " um"       << endl;
  cout << "-------------------------------------------------------------------------------" <<endl;
	return;
}

void circuit::read_iccad2014_file(const char* input)
{
  cout << endl << "Reading .iccad2014 file .." <<endl;
  ...
  cout << "  .iccad2014 file : "<< input <<endl;
  cout << "-------------------------------------------------------------------------------" <<endl;
	
  ifstream dot_iccad2014(input);
  if (!dot_iccad2014.good())
  {
    cerr << "read_iccad2014_file:: cannot open `" << input << "' for reading." << endl;
    exit(1);
  }

	string dot_verilog, dot_sdc, dot_lef, dot_def;
  string filename;
	dot_iccad2014 >> filename;
	while (!dot_iccad2014.eof())
  {
      ...
      {
        cerr << "read_iccad2014_file:: unsupported filetype is specified : " << filename << endl;
        exit(1);
      }
			dot_iccad2014 >> filename;
  }
  ...
  do
  {
    ...
    if (valid)
    {
      ...
      for (unsigned i = 0; i < pinNetPairs.size(); ++i)
      {
        ...
        {
          if(nets[net2id[pinNetPairs[i].second]].source != numeric_limits<unsigned>::max())
          {
            cout << "ERROR: net "<< pinNetPairs[i].second << " is driven by multiple pins! existing.. ";
            exit(3);
          }
          myNet->source = myPin->id;
        }
...
void circuit::read_lef_site(ifstream &is)
{
  ...
  while (tokens[0] != "END")
  {
    ...
    {
      cout << "unsupported keyword " << tokens[0] << endl;
      exit(1);
    }
    get_next_token(is, tokens[0], LEFCommentChar);
  }
...
void circuit::read_lef_macro(ifstream &is)
{
  ...
  while (tokens[0] != "END")
  {
    ...
    {
      cerr << "unsupported keyword " << tokens[0] << endl;
      exit(2);
    }
    get_next_token(is, tokens[0], LEFCommentChar);
  }
    ...
    else if (tokens[0] == "PORT")
    {
      ...
      while (tokens[0] != "END")
      {
        if (tokens[0] == "LAYER")
        {
          ...
          {
            get_next_n_tokens(is, tokens, 2, LEFCommentChar);
            myPin.xLL = atof(tokens[0].c_str());
            if (tokens[1] == "ITERATE")
            {
              get_next_n_tokens(is, tokens, 3, LEFCommentChar);
              myPin.yLL = atof(tokens[0].c_str());
              myPin.xUR = atof(tokens[1].c_str());
              myPin.yUR = atof(tokens[2].c_str());
            }
            else
            {
              myPin.yLL = atof(tokens[1].c_str());
              get_next_n_tokens(is, tokens, 2, LEFCommentChar);
              myPin.xUR = atof(tokens[0].c_str());
              myPin.yUR = atof(tokens[1].c_str());
            }
            get_next_token(is, tokens[0], LEFCommentChar);
            assert(tokens[0] == LEFLineEndingChar);
          }
...
{
  //write .aux
	ofstream dot_aux((benchmark+".aux").c_str());
	if(!dot_aux.good())
		return;
	dot_aux << "RowBasedPlacement : "<< benchmark << ".nodes " << benchmark << ".nets "<< benchmark << ".wts " << benchmark << ".pl " << benchmark << ".scl ";
	dot_aux.close();

	//write .nodes (including PI & PO pins as instances)
	ofstream dot_nodes((benchmark+".nodes").c_str());
	if(!dot_nodes.good())
		return;
	dot_nodes << "UCLA nodes 1.0" << endl << endl;
	dot_nodes << "NumNodes : " << cells.size() + PIs.size() + POs.size() << endl;
	dot_nodes << "NumTerminals : " << PIs.size() + POs.size() + num_fixed_nodes << endl <<endl;
	...
	{
		if(theCell->isFixed)
			dot_nodes << "\t\t" << theCell->name << "\t\t" << theCell->width << "\t\t" << theCell->height << "\t terminal" <<endl;
		else
			dot_nodes << "\t\t" << theCell->name << "\t\t" << theCell->width << "\t\t" << theCell->height <<endl;
	}
	dot_nodes.close();

	//write .nets
	ofstream dot_nets((benchmark+".nets").c_str());
	if(!dot_nets.good())
		return;
	dot_nets << "UCLA nets 1.0" << endl << endl;
	dot_nets << "NumNets : "<< nets.size() << endl;
	dot_nets << "NumPins : "<< pins.size() << endl << endl;
	for(vector<net>::iterator theNet = nets.begin() ; theNet != nets.end() ; ++theNet)
	{
		...
		else
		{
			...
			dot_nets << pins[ theNet->source ].x_offset - 0.5 * pinOwner->width << "\t" << pins[ theNet->source ].y_offset - 0.5 * pinOwner->height <<endl;
		}
		for(vector<unsigned>::iterator theSink = theNet->sinks.begin() ; theSink != theNet->sinks.end() ; ++theSink)
		{
			...
			else
			{
				...
				dot_nets << pins[ *theSink ].x_offset - 0.5 * pinOwner->width << "\t" << pins[ *theSink ].y_offset - 0.5 * pinOwner->height << endl;
			}
		}
	}
	dot_nets.close();
  ...
  //write .wts
	ofstream dot_wts((benchmark+".wts").c_str());
	if(!dot_wts.good())
		return;
  ...
  //write .scl
	...
	ofstream dot_scl(benchmark+".scl");
	if(!dot_scl.good())
		return;
	...
	for(unsigned i=0 ; i<rows.size() ; i++)
	{
		...
		dot_scl << "   Coordinate     : "  << theRow->origY << endl;
		dot_scl << "   Height         : "  << rowHeight << endl;
		dot_scl << "   Sitewidth      : 1" << endl;
		// NOTE: some academic placers do not handle this properly, so set it to 1 here, but multiply it to NumSites
		dot_scl << "   Sitespacing    : 1" << endl;
		dot_scl << "   Siteorient     : N" << endl;
		dot_scl << "   Sitesymmetry   : Y" << endl;
		dot_scl << "   SubrowOrigin   : "  << theRow->origX << "\t\t\tNumSites : " << theRow->numSites * theRow->stepX << endl;
		while(i+1 <= rows.size()-1)
		{
			if(rows[i+1].origY == theRow->origY)
			{
				dot_scl << "   SubrowOrigin   : "  << rows[i+1].origX << "\t\t\tNumSites : " << rows[i+1].numSites * rows[i+1].stepX << endl;
				i++;
			}
			else
				break;
  ...
  //write .pl
	ofstream dot_pl((benchmark+".pl").c_str());
	if(!dot_pl.good())
		return;
	dot_pl << "UCLA pl 1.0" <<endl << endl;
	for(vector<unsigned>::iterator PI=PIs.begin() ; PI!=PIs.end() ; ++PI)
		dot_pl << "\t\t" << pins[ *PI ].name << "\t\t" << pins[ *PI ].x_coord << "\t\t" << pins[ *PI ].y_coord << "\t : N /FIXED" <<endl;
	for(vector<unsigned>::iterator PO=POs.begin() ; PO!=POs.end() ; ++PO)
		dot_pl << "\t\t" << pins[ *PO ].name << "\t\t" << pins[ *PO ].x_coord << "\t\t" << pins[ *PO ].y_coord << "\t : N /FIXED" <<endl;
	for(vector<cell>::iterator theCell = cells.begin() ; theCell != cells.end() ; ++theCell)
	{
		if(theCell->isFixed)
			dot_pl << "\t\t" << theCell->name << "\t\t" << theCell->init_x_coord << "\t\t" << theCell->init_y_coord << "\t : N /FIXED" << endl;
		else
			dot_pl << "\t\t" << theCell->name << "\t\t" << theCell->init_x_coord << "\t\t" << theCell->init_y_coord << "\t : N" << endl;
	}
	dot_pl.close();
	return;
}
  ...
  for(unsigned i=0;i<cells.size();i++)
  {
    ...
    if(it==cell2id.end())
    {
      ...
      exit(1);
    	...
    	dot_def << "      + PLACED ( " << x_coord << " " << y_coord << " ) N ;" <<endl;
    dot_pl >> tmpStr;
    if(tmpStr=="/FIXED")
      isMovable=false;
    else
